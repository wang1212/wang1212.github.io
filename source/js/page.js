...
(function(_w, _d) {
		...
		if ("object" == typeof d && (d.data && (len = d.data.length))) {
			for (var i = 0; i < len; i++) {
				...
				if (expire && tmnow * 1E3 < Date.parse(expire.replace(/\s[\s\S]*$/, "").replace(/\-/g, "/"))) {
					var _c = d.data[i].city,
						_p = d.data[i].province;
					if (_c && city)
					...
					if (_p && province)
						if (("_" + _p + "_").indexOf("_" + province + "_") > -1) ddata.push(d.data[i])
				}
			}
	...
	_w.share = function(target) {
			...
			renren: {
				method: function() {
					var n = "http://www.connect.renren.com/share/sharer?title=" +
						encode(summary) + "&url=" + encode(surl + "#via=share_renren"),
		...
		var openit = function(u, n, p) {
			function o() {
				var z;
				if (!(z = window.open(u, n, p))) location.href = u;
				else z.focus()
			}
			o()
		};
	...
	(function initStat() {
		...
		function cb() {
			...
			var src = "";
			if (qzoneDomainNameRE.test(url)) src = "new404.qzone";
			else if (qqDomainNameRE.test(url)) src = "new404.qq";
			else if (qzsDomainNameRE.test(url)) src = "new404.qzonestyle";
			else src = url.replace(".", "_");
